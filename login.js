document.addEventListener('DOMContentLoaded', () => {
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // FALLBACK FOR GITHUB PAGES (Since no backend runs there)
            console.warn('Backend not detected, running in Frontend-Only Prototype Mode!');
            setTimeout(() => {
                submitBtn.classList.remove('loading');
                sessionStorage.setItem('isLoggedIn', 'true');
                sessionStorage.setItem('currentUserName', document.getElementById('fullname')?.value || 'Demo User');
                sessionStorage.setItem('currentEmail', email);
                sessionStorage.setItem('userFavorites', JSON.stringify([]));
                window.location.replace('index.html');
            }, 800);
        }
    });
